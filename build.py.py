...
def check_row(row, line_number):
    ...
    try:
        ...
        quit_with_error(
            f"Line {line_number} ({row['name']}): price is '{row['price']}'.\n"
            f"Write it as a plain number like 2.50, with no {CURRENCY} sign."
        )
...
def render_item(row):
    ...
    price = f"{CURRENCY}{float(row['price']):.2f}"  # 2.5 becomes $2.50
